    ...
    try {
      ...
      switch (formData.paymentMethod) {
        case 'card': {
          ...
          // Use the new processCardPayment method
          const result = await processCardPayment(orderId, paymentMethod.id);
        ...
        case 'mpesa': {
          if (!formData.mpesaPhone) {
            throw new Error('Please provide a valid M-Pesa phone number');
          }

          // Use the new processMpesaPayment method
          ...
          if (result.success) {
            // For M-Pesa, we'll wait for the callback to update the order status
            // So we can proceed to show the order confirmation
            await onSubmit(formData);
          }
          break;
        }
        ...
        case 'paypal': {
          // Use the new processPayPalPayment method
          const result = await processPayPalPayment(orderId);

          if (result.success) {
            // For PayPal, the user will be redirected to PayPal's site
            // The return URL will handle the order confirmation
        ...
        default:
          throw new Error(
            `Unsupported payment method: ${formData.paymentMethod}`
          );
      }
    } catch (error) {
